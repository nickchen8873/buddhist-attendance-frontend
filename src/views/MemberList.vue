<template>
  <div class="member-list-legacy">
    ...
    <div style="text-align: center;margin-bottom: 35px;">
      ...
      <button class="btn" @click="fetchMembers({hasGroup: 1})">志工幹部</button>
      <button class="btn" @click="fetchRecent3Months()">新進蓮友</button>
      <button class="btn" @click="fetchMembers({showHidden: 1})">解除隱藏</button>
      <input type="text" v-model="keyword" placeholder="輸入法名/姓名/ID後3碼" @keyup.enter="SendKeywordToParent()" style="height: 35px;margin-left: 20px;margin-right: 10px;">
    ...
    <table class="legacy-table">
      ...
      <tbody>
        <tr v-for="m in members" :key="m.id" @dblclick="onEdit(m)" style="cursor: pointer;">
          ...
          <td>{{ m.phone }}</td>
          <td>
            <span v-if="m.address && m.address.length <= 8">{{ m.address }}</span>
            <button class="more-btn" v-else @click="showAddress(m)">更多</button>
          </td>
